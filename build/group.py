...
def _drop_redundant_class(parts: list[str]) -> list[str]:
    ...
    빗금이 든 앞머리도 같은 것이다. '파이/만주'·'비스킷/쿠키/크래커' 는 음식
    이름이 아니라 식약처가 쓰는 서랍 이름이라, 사과파이를 검색한 사람에게
    '파이/만주 사과파이' 라고 보여주면 정작 찾은 이름이 뒤에 가 붙는다.
    빗금이 조각 하나뿐인 이름('리소토/리조또')이면 그것이 이름이므로 둔다.
    괄호 안의 빗금('소고기(동그랑땡/육원전)')은 병기지 분류가 아니다.
    """
